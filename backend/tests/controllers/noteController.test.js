...
describe('Note Controller Unit Tests', () => {
  ...
  describe('POST /api/notes', () => {
    ...
    it('should create a note successfully', async () => {
      ...
      expect(response.body.content).to.equal('This is a new note');
    });
    ...
    it('should return 400 when title is missing', async () => {
      ...
      const response = await request(app)
        .post('/api/notes')
        .send({
          content: 'I forgot the title'
        });

      expect(response.status).to.equal(400);
      expect(response.body.message)
        .to.equal('Please provide both title and content');
    });
    ...
    it('should return 400 for invalid note parameters', async () => {
      ...
      expect(response.status).to.equal(400);
      expect(response.body.message)
        .to.equal('Invalid note parameters');
    });
  ...
  describe('PUT /api/notes/:id', () => {
    ...
    it('should return 404 if note is not found', async () => {
      ...
      expect(response.body.message)
        .to.equal('Note not found or not authorized');
    });
  ...
  describe('DELETE /api/notes/:id', () => {
    ...
    it('should delete a note successfully', async () => {
      ...
      const note = {
        ...
        content: 'Test content',
        user: {
          toString: () => '60d0fe4f5311236168a109ca'
        },
      ...
      expect(response.body.message).to.equal('Note removed');
    });
